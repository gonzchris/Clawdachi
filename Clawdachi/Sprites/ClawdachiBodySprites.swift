...
class ClawdachiBodySprites {
    ...
    // MARK: - Pirate Limb Textures

    private static let pirateShirt = PixelColor(r: 245, g: 235, b: 220)
    private static let pirateShirtDark = PixelColor(r: 200, g: 190, b: 175)
    private static let pirateShirtLight = PixelColor(r: 255, g: 250, b: 245)
    private static let piratePants = PixelColor(r: 60, g: 50, b: 45)
    private static let piratePantsDark = PixelColor(r: 40, g: 30, b: 25)
    private static let pirateBoot = PixelColor(r: 50, g: 35, b: 25)
    private static let pirateBootDark = PixelColor(r: 30, g: 20, b: 15)

    /// Generate pirate left arm texture (3x3 pixels) - cream shirt sleeve
    static func generatePirateLeftArmTexture() -> SKTexture {
        var pixels = Array(repeating: Array(repeating: P.clear, count: 3), count: 3)

        // Row 2 (top/inner) - cream shirt
        pixels[2][0] = pirateShirtDark
        pixels[2][1] = pirateShirt
        pixels[2][2] = pirateShirt

        // Row 1 (middle) - cream shirt
        pixels[1][0] = pirateShirtDark
        pixels[1][1] = pirateShirt
        pixels[1][2] = pirateShirt

        // Row 0 (bottom/outer) - shirt cuff
        pixels[0][0] = pirateShirtDark
        pixels[0][1] = pirateShirtLight
        pixels[0][2] = pirateShirtLight

        return PixelArtGenerator.textureFromPixels(pixels, width: 3, height: 3)
    }

    /// Generate pirate right arm texture (3x3 pixels) - cream shirt sleeve
    static func generatePirateRightArmTexture() -> SKTexture {
        var pixels = Array(repeating: Array(repeating: P.clear, count: 3), count: 3)

        // Row 2 (top/inner) - cream shirt
        pixels[2][0] = pirateShirt
        pixels[2][1] = pirateShirt
        pixels[2][2] = pirateShirtLight

        // Row 1 (middle) - cream shirt
        pixels[1][0] = pirateShirt
        pixels[1][1] = pirateShirt
        pixels[1][2] = pirateShirtLight

        // Row 0 (bottom/outer) - shirt cuff
        pixels[0][0] = pirateShirtLight
        pixels[0][1] = pirateShirtLight
        pixels[0][2] = pirateShirtDark

        return PixelArtGenerator.textureFromPixels(pixels, width: 3, height: 3)
    }

    /// Generate pirate left leg texture (2x5 pixels) - dark pants with boots
    static func generatePirateLeftLegTexture() -> SKTexture {
        var pixels = Array(repeating: Array(repeating: P.clear, count: 2), count: 5)

        // Row 4 (top) - dark pants
        pixels[4][0] = piratePantsDark
        pixels[4][1] = piratePants

        // Row 3 - pants
        pixels[3][0] = piratePantsDark
        pixels[3][1] = piratePants

        // Row 2 - pants meet boots
        pixels[2][0] = piratePantsDark
        pixels[2][1] = piratePants

        // Rows 0-1 (bottom) - brown boots
        pixels[1][0] = pirateBoot
        pixels[1][1] = pirateBoot
        pixels[0][0] = pirateBootDark
        pixels[0][1] = pirateBootDark

        return PixelArtGenerator.textureFromPixels(pixels, width: 2, height: 5)
    }

    /// Generate pirate right leg texture (2x5 pixels) - dark pants with boots
    static func generatePirateRightLegTexture() -> SKTexture {
        var pixels = Array(repeating: Array(repeating: P.clear, count: 2), count: 5)

        // Row 4 (top) - dark pants
        pixels[4][0] = piratePants
        pixels[4][1] = piratePants

        // Row 3 - pants
        pixels[3][0] = piratePants
        pixels[3][1] = piratePants

        // Row 2 - pants meet boots
        pixels[2][0] = piratePants
        pixels[2][1] = piratePantsDark
